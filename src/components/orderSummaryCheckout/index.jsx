...
import PaymentPop from "@components/popUp/PaymentPop"
const lastCallCache = new Map(); // key: payload signature, value: timestamp
const isTabbyPaymentEnabled = process.env.NEXT_PUBLIC_SHOW_TABBY_PAYMENT === 'true';
...
const OrderSummaryComp = ({ dataRec }) => {
  ...
  const [promoStatuselocal, setPromoStatuselocal] = useState('');
  const [promoStatuselocalonceCall, setPromoStatuselocalonceCall] = useState('once');
  ...
  const wallet_deduction = get(orderSummaryLocal, "wallet_deduction", null)
  ...
  */
  ...
  const [ModalTabbyPaymentOpen,setModalTabbyPaymentOpen]=useState(false);
  ...
  const DisplayTotal = get(orderSummaryLocal, "display_values.total", 0)
  const DisplaySubTotal = get(orderSummaryLocal, "display_values.subtotal", 0)
  const DisplayDiscountAmount = get(orderSummaryLocal, "display_values.discount_amount", 0)
  const DisplayWalletDeduction = get(orderSummaryLocal, "display_values.wallet_deduction", 0)
  const DisplayWeekDiscountAmount = get(orderSummaryLocal, "display_values.week_discount_amount", 0)
  const DisplayAdditionalServiceAmount = get(orderSummaryLocal, "display_values.additional_services_amount", 0)
...
const checkIfCapped = (
  ...
  const handleDeleteCardRequest = (card_id) => {
    dispatch(deleteCardRequest({ accessToken, card_id }))
      ...
      .then((res) => {
        ...
        setCheckoutBody({ ...checkoutBody })
        handleGetOrderSummaryRequest(router.query.order_id ?? userProfile?.order?.id,
              promoCodeSummaryLocal?.id)
      })
      .catch((err) => {
  ...
  const handleAddAddressRequest = (addressBody) => {
    ...
    dispatch(addAddressRequest({ accessToken, addAddressBody }))
      ...
      .then((res) => {
        ...
        handleGetOrderSummaryRequest(
          router.query.order_id ?? userProfile?.order?.id,
          promoCodeSummaryLocal?.id
        )
  ...
  const handleGetOrderSummaryRequest = async (order_id, discountId = null) => {
    try {
    ...
    try {
      ...
      if (checkoutBody) {
        ...
        let newCheckoutBody = {
          ...
          price: Number(roundHalfDown(Number(DisplayTotal)).toFixed(0)),
          ...
          topUpStatus == "top up" ? null : promoCodeSummaryLocal?.id ? promoCodeSummaryLocal?.id : checkoutBody.discount_id,
        ...
        if(localDataToSave?.tabby && isTabbyPaymentEnabled){
          delete newCheckoutBody['card_id']
          newCheckoutBody = {
            ...newCheckoutBody,
            checkout_type: 'tabby',
          }
        ...
        //  const paymentUrl =  "https://uae.paymob.com/unifiedcheckout/?publicKey=are_pk_live_XBtgqcFSkcLxXvR68MMImvnGXM4ZazUb&clientSecret=are_csk_live_ec3c5c488059e24575851b8533461bed"; // depends on your API response
        //     if(paymentUrl){
        //       showSuccessToast("Currently Paymob work is in progress")
        //       setIframeUrl(paymentUrl)
        //       setPaymentURL(paymentUrl)
        //       setModalTabbyPaymentOpen(true)
        ...
        dispatch(
          ...
          .then((res) => {
            ...
            // Paymob tabby work
            const paymentUrl = res?.data?.payment_url ?? "https://uae.paymob.com/unifiedcheckout/?publicKey=are_pk_live_XBtgqcFSkcLxXvR68MMImvnGXM4ZazUb&clientSecret=are_csk_live_ec3c5c488059e24575851b8533461bed"; // depends on your API response
            if(paymentUrl && isTabbyPaymentEnabled){
              showSuccessToast("Redirecting to tabby gateway. Please wait...")
              // setIframeUrl(paymentUrl)
              // setPaymentURL(paymentUrl)
              // setModalTabbyPaymentOpen(true)
              window.location.href = res?.data?.payment_url;
  ...
  const handleCheckOutRequest = (requesttype) => {
    try {
      ...
      const saveAndProceed = () => {
        ...
        dispatch(saveOrderLocalSummary(localDataToSave))
          ...
          .then(res => {
            ...
            goToCompleteOrderRequest({...localDataToSave,tabby:requesttype?.tabby});
            // goToCompleteOrderRequest(false);
          })
          .catch(err => AppLogger("this is err", err));
      };
      if(cardDefaultLocal?.id == null && !requesttype?.tabby ){
...
// that can be subtracted from totalPayable. Returning a fixed float is recommended.
      ...
      await dispatch(
        ...
        .then((res) => {
          ...
          if (res.data) {
            ...
            setPromoApplied(true)
            if(promoStatuselocalonceCall == 'once'){
              handleGetOrderSummaryRequest(router.query.order_id ?? userProfile?.order?.id,res?.data?.id)
            }
            setPromoStatuselocalonceCall('')
          } else {
            setPromoCode("")
    ...
    try {
      if (accessToken && payload && active_order_id) {
        ...
        dispatch(updateDeliveryAddressWithDaysRequest({
          ...
          .then(unwrapResult)
          .then((response) => {
            handleGetOrderSummaryRequest(router.query.order_id, promoCodeSummaryLocal?.id);
            // Success Toast for changes in addresses
            showSuccessToast(AppConstants.checkoutAddressToastMessage);
            getAddressWithDays()
            setIsLoading(false);
  ...
  useEffect(() => {
    if (
      ...
      try{
        console.log("11")
        const payload = {
         promoCode: promoCode || orderSummaryLocal?.discount?.promo_code || userProfileFordiscount?.promo_code  ,
         is_subscribed: checkoutBody.is_subscribed,
         order_id: router.query.order_id
       }
        console.log("payload",payload)
         const promoValueMustNotBe = promoStatuselocal != 'removed' && promoStatuselocal != 'applied' && promoStatuselocal != 'checkout';
         const oldCondition = userProfileFordiscount?.promo_code || promoCode || orderSummaryLocal?.discount?.promo_code;
        ...
        if(orderSummaryLocal?.discount){
            oldCondition && promoValueMustNotBe && setloaderFirsttimeDisc(true);
            oldCondition && promoValueMustNotBe && handleGetOrderDiscount(payload).then(()=>{
             setloaderFirsttimeDisc(false);}).catch((err)=>{
               setloaderFirsttimeDisc(false);
             })
        }else{
          if(userProfileFordiscount?.promo_code && promoCode =='' && orderSummaryLocal?.discount == null){
            oldCondition && promoValueMustNotBe && setloaderFirsttimeDisc(true);
            oldCondition && promoValueMustNotBe && handleGetOrderDiscount(payload).then(()=>{
             setloaderFirsttimeDisc(false);
            }).catch((err)=>{
  ...
  }, [userProfileFordiscount, totalWeeks, orderType, checkoutBody.is_subscribed, orderSummaryLocal ,promoStatuselocal])
  useEffect(() => {
    if (
      orderSummaryLocal &&
  ...
  //first
// 1. Initialize a ref at the top of your component
const hasFetchedSummary = useRef(false);

useEffect(() => {
  // 2. Check if the ID exists AND if we haven't fetched it yet
  if (router.query.order_id && !hasFetchedSummary.current) {
    ...
    if(promoCodeSummaryLocal?.id == null){
        orderSummaryParams.promo_code = '';
    }
    ...
    // 3. Set the ref to true so this block never runs again
    hasFetchedSummary.current = true;
  }
}, [dataUpdated, router.isReady, router.query, promoCodeSummaryLocal?.id]);
useEffect(() => {
  if(hasFetchedSummary.current && promoStatuselocal){
    // if(promoStatuselocal != 'removed'){
    // }
    if(promoStatuselocal == 'checkout'){
      return
    }
    setTimeout(() => {
      handleGetOrderSummaryRequest(router.query.order_id, promoCodeSummaryLocal?.id ?? null);
    }, 100);
  }
}, [promoCodeSummaryLocal?.id,router.query.order_id , hasFetchedSummary.current, promoStatuselocal]);
  ...
  const removeAppliedPromo =async()=>{
    setPromoStatuselocal('removed')
    setPromoCodeWalletCredit(null)
    setOrderSummaryParams({...orderSummaryParams,promo_code:''})
    setpromocodeLoader(false)
    setPromoApplied(false)
    setPromoCodeSummaryLocal(null)
    setPromoCode("")
  ...
  */
  ...
  const handleAddItemModalOpen = () => {
    setShowAdditemModal(true);
     ...
     const lodader = loaderFirsttimeDisc || orderSummeryOnlyLoader;
 
      useEffect(() => {
        // Check if everything is actually ready
        const isEverythingReady = !(
          loaderFirsttimeDisc ||
          orderSummeryOnlyLoader 
        );
      ...
      }, [loaderFirsttimeDisc, orderSummeryOnlyLoader]);
  
      useEffect(() => {
        return () => {
          setPromoStatuselocal('')
          removeAppliedPromo()
          setPromoStatuselocalonceCall('')
        }
      }, [])
  return (
    <>
      ...
      <div className={`checkout--pageSummary ${isExecutive ? "isExecutive" : ""} `}>
                  ...
                  <div className={styles.price_wrapper}>
                    <div className={styles.details_wrapper}>
                      ...
                      >
                        ...
                        AED {Number(DisplaySubTotal).toFixed(2)}
                      </Typography>
                    </div>
                    {desclaimer ? (
                      <div>
                    ...
                    {/* {week_discount_details && !discountSummaryLocal ? (
                      <div>
                        <Typography
                          ...
                          color={AppColors.primaryGreen}
                        >
                          {`You saved ${roundHalfDown(week_discount_amount)} AED as ${display_key}` ?? ""}
                        </Typography>
                      </div>
                    ) : null} */}
                    {/* {promoCodeWalletCredit ? (
                        ...
                        <>
                          <div className={styles.details_wrapper}>
                            ...
                            >
                              ...
                              AED {roundHalfDown(Number(DisplayDiscountAmount)).toFixed(2)}
                            </Typography>
                          </div>
                        </>
                      )}
                    ...
                    <div className={styles.discount_wrapper}>
                      <div className={styles.details_wrapper}>
                        ...
                        <Typography
                          ...
                          fontWeight={600}
                        >
                          AED{''} {Number(DisplayWeekDiscountAmount).toFixed(2)}
                        </Typography>
                      </div>
                    </div>
                  ) : null}
                  {DisplayWalletDeduction ? (
                    <div className={styles.discount_wrapper}>
                      <div className={styles.details_wrapper}>
                        <Typography
                          variant="body3"
                        ...
                        <Typography
                          ...
                          fontWeight={600}
                        >
                          AED{''} {Number(roundHalfDown(Number(DisplayWalletDeduction))).toFixed(2)}
                        </Typography>
                      </div>
                    </div>
                  ) : null}
                  ...
                  <div className={styles.total_wrapper}>
                    <div className={styles.details_wrapper}>
                      ...
                      >
                        ...
                        roundHalfDown(Number(DisplayTotal)).toFixed(2)
                        }
                  ...
                  {/* {
                    week_discount_details && !discountSummaryLocal &&
                      <Typography
                            ...
                            className={styles.heading}
                          >
                            {`You just saved ${roundHalfDown(DisplayWeekDiscountAmount)} AED by choosing your plan smartly — with our limited time discounts`}
                          </Typography>
                  } */}
            ...
            <div className={styles.paymentWrapper}>
              ...
              <div style={{border:isExecutive? "1px solid #fa7324":""}} className={styles.priceBox}>
                <PriceBox
                  ...
                  dataRec={dataRec}
                  secStyle={true}
                  tabbycondition={meal_plan_require_weeks == 4 && radioVal != 1 && conditionForPaymob && isTabbyPaymentEnabled}
                  placeOrderRequest={() => console.log("this is clicked")}
                  clicked={handleCheckOutRequest}
                  price={
                    roundHalfDown(Number(DisplayTotal)).toFixed(0)
                  }
                />
        ...
        {/* {
          ModalTabbyPaymentOpen &&
        <PaymentPop
          open={ModalTabbyPaymentOpen}
          onClose={()=>setModalTabbyPaymentOpen(false)}
          IframeUrl={IframeUrl} 
